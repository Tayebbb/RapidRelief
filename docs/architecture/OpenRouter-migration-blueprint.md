...
2. **Breaker classification gaps** — the 200-with-error and `finish_reason:"error"` paths must throw *Unavailable* (count), while 403/`content_filter` must *not*; a missed catch-order (`AiProviderBlockedException` after the generic catch) silently reverts D-064.
